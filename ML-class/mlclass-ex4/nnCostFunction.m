...
J -= lambda/2*sum(sum(Theta1(:, 2:size(Theta1,2)).^2));
J -= lambda/2*sum(sum(Theta2(:, 2:size(Theta2,2)).^2));
J /= -m;
...
Theta2_grad = Delta2 + Theta2*lambda/m;
Theta1_grad = Delta1 + Theta1*lambda/m;
